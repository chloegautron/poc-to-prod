...
class BaseTextCategorizationDataset:
    ...
    def get_label_to_index_map(self):
        ...
        """
        return {j:i for i,j in self.get_index_to_label_map().items()}
    ...
    def __repr__(self):
        return self.__class__.__name__ + \
               ...
               f"n_labels: {self._get_num_labels()})"
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        assert (dataset.columns == ['post_id','tag_name','tag_id','tag_position','title']).all()
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                new_df = df[df.tag_name.map(df['tag_name'].value_counts()) >= x]
                return new_df
        ...
        preprocess_data = dataset.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label))
    ...
    def get_train_batch(self):
        ...
        next_x = self.x_train[i * self.batch_size : (i+1) * self.batch_size].apply(self.preprocess_text)
