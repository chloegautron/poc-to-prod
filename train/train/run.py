...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    shape_of_input = (dataset.get_train_batch()[0].shape[-1],)

    model = Sequential([
        Dense(train_conf['dense_dim'], activation='relu', input_shape=shape_of_input),
        Dense(dataset._get_num_labels(), activation='sigmoid')
    ])
    ...
    with open(artefacts_path +"/labels_index.json", "w") as outfile:
        json.dump(dataset.get_label_to_index_map(), outfile)
    ...
    return accuracy, artefacts_path
